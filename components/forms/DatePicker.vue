<template>
  <div>
    <label v-if="label" :for="dateId" class="block font-medium mb-2">
      {{ label }}
    ...
    <div class="relative">
      ...
      <div v-if="!disabled" class="absolute right-3 top-1/2 -mt-2.5 pointer-events-none text-black bg-white pl-1 transition-all peer-focus:translate-x-[4px] peer-focus:translate-y-[4px]">
        <IconLucid name="Calendar" size="sm" :strokeWidth="2" />
      </div>
    </div>
...
const inputClasses = computed(() => {
  const base = 'w-full h-12 pl-4 pr-10 py-3 border-2 bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] focus:outline-none focus:shadow-none focus:translate-x-[4px] focus:translate-y-[4px] transition-all';
